...
module "landing-to-onprem-uc1-vpn" {
  ...
  router_asn            = var.router_onprem_configs.landing-uc1.asn
  peer_external_gateway = var.vpn_onprem_configs.landing-uc1.peer_external_gateway
  tunnels = {
    for t in var.vpn_onprem_configs.landing-uc1.tunnels :
    "remote-${t.vpn_gateway_interface}-${t.peer_external_gateway_interface}" => {
      bgp_peer = {
        address = cidrhost(t.session_range, 1)
        asn     = t.peer_asn
      }
      bgp_peer_options                = local.bgp_peer_options_onprem.landing-uc1
